...
<table lay-filter="demo" id="demo">
    ...
    <thead>
    <tr>
        <th lay-data="{field:'1',sort:true,align:'center'}">工号</th>
        <th lay-data="{field:'2',align:'center'}">姓名</th>
		<th lay-data="{field:'3',align:'center'}">联系方式</th>
        <th lay-data="{field:'4', toolbar:'#userBar' ,width:200,align:'center', fixed:'right'}">操作</th>
    </tr>
    </thead>
    <tbody>
    <c:forEach items="${SESSION_TEA}" var="user">
        <tr>
            <td>${user.id}</td>
            <td>${user.name}</td>
			<td>${user.phone}</td>
